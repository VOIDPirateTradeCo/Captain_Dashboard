"""Standalone Miss Pink Discord bot — containerized, restart:unless-stopped."""
...
# === CONFIG ===
TOKEN = os.environ.get("DISCORD_MISS_PINK_TOKEN") or os.environ.get("DISCORD_MISS_PINK_TOKEN")
...
STATE_PATH = pathlib.Path(os.environ.get("STATE_PATH", "/state/miss_pink_bot_state.json"))
...
if not TOKEN:
    raise SystemExit("Missing DISCORD_MISS_PINK_TOKEN or DISCORD_MISS_PINK_TOKEN")
...
def _call_hermes(prompt: str) -> str:
    try:
        ...
        fname = f"{ts}_miss_pink.json"
        data = {"prompt": prompt, "created_at": time.strftime("%Y%m%dT%H%M%S")}
        (base / "inbound" / "miss_pink" / fname).write_text(
            json.dumps(data, indent=2), encoding="utf-8"
        )
        out_path = base / "outbound" / "miss_pink" / fname.replace(".json", "_reply.json")
...
async def on_ready() -> None:
    print(f"[READY] Miss Pink bot online as {bot.user}", flush=True)
    try:
...
@bot.tree.command(name="ping", description="Check Miss Pink bot latency")
...
async def on_message(message: discord.Message) -> None:
    ...
    if message.guild is not None and bot.user in message.mentions:
        ...
        if cleaned:
            ...
            try:
                await message.reply("Acknowledged. Send a task after the @Miss Pink mention.", mention_author=False)
            except Exception as e:
...
def main() -> None:
    print("[START] Launching Miss Pink Discord bot...", flush=True)
