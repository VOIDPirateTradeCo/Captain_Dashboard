#!/usr/bin/env python3
"""OODA Batch C: close remaining VERIFIED-DONE actionable cards (no regression)."""
import urllib.request as u, urllib.parse as p, json, time
SRC = r"C:\Users\kidsm\Documents\My Docs\VOID Pirate Trading Co\SECRETS_ENV"
env={}
for line in open(SRC,encoding='utf-8'):
    if line.strip().startswith('TRELLO_') and '=' in line:
        k,v=line.strip().split('=',1); env[k]=v.strip()
KEY,TOK=env['TRELLO_KEY'],env['TRELLO_TOKEN']
BASE="https://api.trello.com/1"
def call(path, data=None, method='GET', tries=5):
    url=f"{BASE}{path}?key={KEY}&token={TOK}"
    if data: url+="&"+p.urlencode(data)
    for i in range(tries):
        try:
            req=u.Request(url, method=method, data=b'' if method in ('POST','PUT') else None)
            return json.loads(u.urlopen(req,timeout=15).read())
        except Exception as e:
            time.sleep(3*(i+1)); last={"error":str(e)[:120]}
    return last
def close(cid, why):
    r=call(f"/cards/{cid}", {"closed":"true"}, method='PUT')
    print(f"  close {cid[:8]}: {'OK' if r.get('closed') else r.get('error','?')}  ({why})"); time.sleep(1.3)

DONE = [
    "6a758f3514bf80228dd1c5a0",  # STEALTHATTACK mesh integration -> done (mesh live)
    "6a73be557a811d8436972753",  # tools_security_tabs_and_mode_choice -> done (security tab)
    "6a743ac02f44b8d88792c908",  # speed up stat refresh + dashboard alerting -> done (prewarm+watchdog)
    "6a76c7216c9f9f244a350620",  # Crew install hive agent 3 rigs -> done (mesh on 3 rigs live)
    "6a77809af89985029beb18f0",  # Docker extension rollout + fleet topology -> done (locked)
]
print("=== Batch C: close verified-done ===")
for cid in DONE:
    close(cid, "verified complete (Sir Green)")
print("BATCH C DONE")
