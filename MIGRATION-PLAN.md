...
- **`void-mc-passwd.sh` — admin credential reset tool (Sir Cobalt, 2026-08-31).** MC has no in-app "forgot password" and no CLI user command; the admin lives as a scrypt hash in the container DB. New wrapper next to `void-mc.sh`: `list` (show users, no secrets), `set <user> <pass>` (reset/create, hash + params match `src/lib/password.ts` — proven to round-trip through MC's `verifyPassword`), `reset` (clear `users` so `/setup` reopens). Snapshots the DB to `.backups/` before every mutation. `list` tested live.
- **E2E re-verification pass, 2026-08-31T23:33Z (Sir Cobalt).** Against the *running* stack (not just clean builds): `dashboard_server.py` live on :8080 → `GET /api/monitor` returns the full rollup (services 4/4, `alerts.firing=4` incl. 2 critical, `targets` 6/4-up/2-down, `mc_agents` 6/0-online, `tr3asure_fleet` degrades cleanly, `verdict="red"`); `GET /api/prometheus` → real `alerts:4` (no more hardcoded 0). `void-prometheus` → all 10 `fleet_alerts` rules loaded, `health=ok`, 4 actively firing on the real 2-node `windows_exporter` outage. WHITE WHALE: `grep VOID_PIRATE_BLACK_7734|passphrase_hint` across `Captain_Dashboard/` = 0; live `/api/tools` response carries neither. Cards 12158/12159/12160 are code-complete + evidence-backed; 12161 panel render still waits on the MC image rebuild (running container is a pre-panel build).
