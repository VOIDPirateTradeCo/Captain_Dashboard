param(
  ...
  [int]$MasterPort = 3100,
  [string]$ShipName = 'stealthattack',
  [string]$ShipPort = '3000',
  [string]$WorkDir = 'S:\Sir_Azure\mission-control',
  [string]$McApiKey = '',
...
Write-Host '=== STEALTHATTACK Fleet Bootstrap ===' -ForegroundColor Cyan

# 0. Working directory sanity
...
# 4. Open ship-side firewall for master LAN
Write-Host '--- FIREWALL ---'
netsh advfirewall firewall add rule name="ALLOW-MC-STEALTHATTACK-3000" dir=in action=allow protocol=TCP localport=3000 remoteip=${MasterHost} profile=private | Out-Null
netsh advfirewall firewall show rule name="ALLOW-MC-STEALTHATTACK-3000" | Select-String 'Rule Name|Enabled|Profiles|LocalPort|RemoteIP|Action'

# 5. Start MC dev server
...
# 8. Register / heartbeat against master using adapter API
if ($McApiKey) {
