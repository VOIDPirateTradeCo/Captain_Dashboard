...
/**
 * VOID Hive Health panel — one "is the crew hive-mind healthy?" view.
 *
 * Data: GET /api/void-proxy/monitor -> dashboard_server.py handle_monitor_api()
 * on the host (0.0.0.0:8080). That endpoint aggregates service reachability,
 * live Prometheus alerts (from dashboard/monitoring/fleet_alerts.yml),
 * scrape-target up/down, the Mission Control agent roster, tr3asure fleet
 * connectivity, and a green|amber|red verdict. Every source degrades
 * independently, so partial data is normal.
 *
 * Cards: VOID Ops 12159 (alert rules) + 12160 (this rollup).
 */
  ...
  return (
    <div className="p-6 space-y-6 max-w-5xl">
      <div className="flex items-center justify-between">
        ...
        <Button variant="outline" size="sm" onClick={() => mon.reload()}>Refresh</Button>
      </div>

      {mon.err && <p className="text-xs text-red-500">collector: {mon.err}</p>}
      {d?.generated_at && (
        <p className="text-[11px] text-muted-foreground -mt-4">as of {d.generated_at}</p>
      )}
      ...
      <div className="rounded-lg border border-border bg-card p-4">
        ...
        <div className="mt-2 grid gap-2 sm:grid-cols-2 md:grid-cols-4">
          ...
          {services.length === 0 && <p className="text-xs text-muted-foreground">{mon.loading ? 'loading…' : 'no data'}</p>}
        </div>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <div className="rounded-lg border border-border bg-card p-4">
          <div className="flex items-center justify-between">
            ...
            <span className="text-xs text-muted-foreground">
              {alerts?.ok
                ? `${alerts.firing ?? 0} firing · ${alerts.pending ?? 0} pending`
                : (alerts?.error || '—')}
            </span>
          </div>
        ...
        <div className="rounded-lg border border-border bg-card p-4">
          <div className="flex items-center justify-between">
            ...
            <span className="text-xs text-muted-foreground">
              {targets?.ok ? `${targets.up ?? 0}/${targets.total ?? 0} up` : (targets?.error || '—')}
            </span>
          </div>
          <div className="mt-2 space-y-1">
      ...
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <div className="rounded-lg border border-border bg-card p-4">
          <div className="flex items-center justify-between">
            ...
            <span className="text-xs text-muted-foreground">
              {agents?.ok ? `${agents.online ?? 0}/${agents.total ?? 0} online` : (agents?.error || '—')}
            </span>
          </div>
          <div className="mt-2 space-y-1">
