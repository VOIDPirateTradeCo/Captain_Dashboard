...
try {
  // Force WAL checkpoint to sync changes
  db.pragma('wal_checkpoint(TRUNCATE)');
  console.log('WAL checkpointed');
  
  // Verify crew agents
  const rows = db.prepare("SELECT id, name, status, last_seen FROM agents WHERE name IN ('sir-green', 'sir-cobalt', 'sir-azure', 'miss-pink')").all();
