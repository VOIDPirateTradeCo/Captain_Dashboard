...
const CARD_IDS = [
  '6a97ba8ccfa8fbbc6ebccf96',
  '6a97bddda528df1b6692de4b',
  '6a97bdddbe9bcb98cfc03230',
]
...
async function main() {
  ...
  const doneList = (listRes.data || []).find(l => /done/i.test((l && l.name) || ''))
  const doneListId = doneList?.id
  if (!doneListId) { console.error('Done list not found'); process.exit(1) }

  for (const cardId of CARD_IDS) {
    const commentRes = await req(`/cards/${cardId}/actions/comments`, { method: 'POST', body: { text: 'Verified security/sandbox fixes already applied in ~/.openclaw/openclaw.json: tools.deny + sandbox.mode=all, build exits 0, backup present at openclaw.json.bak.sir-green-20260902.' } })
    console.log('Evidence', commentRes.status, cardId, commentRes.text || JSON.stringify(commentRes.data).slice(0,200))
    const moveRes = await req(`/cards/${cardId}?idList=${doneListId}`, { method: 'PUT' })
    console.log('Move', moveRes.status, cardId, moveRes.data?.idList)
  }
}
