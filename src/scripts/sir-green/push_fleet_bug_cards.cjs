...
async function main() {
  ...
  const listName = "Sir Green's Inbox"
  const cards = JSON.parse(fs.readFileSync(path.join('src', 'scripts', 'sir-green', 'fleet-bug-cards.json'), 'utf8'))

  const listRes = await request(`/boards/${boardId}/lists?fields=name,id`)
  const list = JSON.parse(listRes.body).find((l) => l.name === listName)
  if (!list) throw new Error(`List "${listName}" not found on board ${boardId}`)

  const labelRes = await request(`/boards/${boardId}/labels?fields=name,id`)
  const labelMap = new Map()
  for (const label of JSON.parse(labelRes.body)) {
  ...
  for (const card of cards) {
    ...
    const res = await post('/cards', createBody)
    if (res.status >= 400) {
