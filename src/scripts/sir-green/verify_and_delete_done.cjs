...
async function main() {
  ...
  for (const card of toVerify) {
    try {
      ...
      if (hasEvidence) {
        // Delete the card
        const deleteResult = await trelloDelete(`/cards/${card.id}`);
        if (deleteResult.status === 200) {
          deleted++;
          if (deleted % 10 === 0) console.log(`Deleted: ${deleted}`);
        } else {
          errors++;
          console.log(`Delete failed for ${card.id}: ${deleteResult.status}`);
        }
      } else {
        skipped++;
      }
      ...
      errors++;
      console.log(`Error processing ${card.id}: ${e.message}`);
    }
  }
