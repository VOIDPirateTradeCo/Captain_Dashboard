...
async function main() {
  ...
  const mcCards = cards.filter(c => {
    ...
    const isMC = name.includes('mission control') || name.includes('[mc]') || name.includes('mc ') || 
                 ...
                 name.includes('heartbeat') || name.includes('service');
    return isMC && (isP1 || isP2 || isAction || isTask) && !labels.includes('Done') && c.idList !== '6a595669b8f8f99c93392f6c';
  });
  
  console.log(`Remaining MC P1/P2/Action cards: ${mcCards.length}`);
  ...
  // Total open on board
  const totalOpen = cards.filter(c => !c.closed);
  console.log(`\nTotal open cards on board: ${totalOpen.length}`);
  const totalMC = mcCards.length;
  console.log(`MC-related P1/P2/Action still open: ${totalMC}`);
}
