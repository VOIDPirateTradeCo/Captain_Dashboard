...
async function main() {
  ...
  // Get all cards and find MC P1/P2/Action cards
  ...
  const mcCards = cards.filter(c => {
    ...
    const isMC = name.includes('mission control') || name.includes('[mc]') || name.includes('mc ') || 
                 ...
                 name.includes('install') || name.includes('windows');
    return isMC && (isP1 || isP2 || isAction || isTask) && !labels.includes('Done');
  });
  
  console.log(`MC P1/P2/Action cards (${mcCards.length}):`);
